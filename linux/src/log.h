#pragma once
//
// log — stderr logger with level gating.
//
// Mirrors the Windows helper's log API (LH_LOG_INFO / LH_LOG_DEBUG / ...)
// so dispatch_codec_test and other cross-platform sources port without
// macro changes. The implementation is Linux-flavoured:
//   - Uses fwrite to stderr; localtime_r for timestamping; getpid().
//   - DEBUG level gated by SetVerbose(true) (set from main.cpp when the
//     --verbose flag is passed).
//
// FormatLogLine is a pure function so log_test can exercise the layout
// without touching real I/O or the wall clock.
...
// Toggle DEBUG-level emission. Default off; --verbose flips it on. Safe to
// call from any thread (atomic), but typically called once during startup.
