// ExtClipboardManager — drives the ext-data-control-v1
...
//     lives in a heap-allocated `ExtState` held via shared_ptr.
...
//   1. ext-data-control requires a FRESH ext_data_control_source_v1
...
std::vector<std::uint8_t> ConvertImageForMime(
    ...
    if (!img.loadFromData(webp_bytes.data(),
        ...
        LH_LOG_WARN("[ext/image] QImage::loadFromData failed; "
    ...
    if (fmt == nullptr) {
        std::ostringstream m;
        m << "[ext/image] unsupported image MIME '" << mime
    ...
    if (!img.save(&buf, fmt)) {
        std::ostringstream m;
        m << "[ext/image] QImage::save as " << fmt << " failed";
        LH_LOG_WARN(m.str());
        return {};
    }
...
void WaitAndWriteOnWorker(std::shared_ptr<ExtState> state,
    ...
    switch (outcome) {
        ...
        case Outcome::Stale: {
            std::ostringstream m;
            m << "[ext] send for hash=" << ShortHash(hash)
        ...
        case Outcome::Timeout: {
            std::ostringstream m;
            m << "[ext] PROVIDE_DATA timed out after "
...
// closes fd.
void DrainAndDispatchOnWorker(std::shared_ptr<ExtState> state, int fd) {
...
// the anonymous one) so MakeExtManager — the public entry point
// declared in clipboard_manager.h — can name the type.

class ExtClipboardManager : public QObject, public ClipboardManager {
public:
    ExtClipboardManager() : state_(std::make_shared<ExtState>()) {}

    ~ExtClipboardManager() override {
        // Wake any detached workers. They hold a shared_ptr<ExtState>
    ...
    bool Init() {
        ...
        if (display_->manager_ext() == nullptr) {
            LH_LOG_DEBUG("[ext] compositor did not advertise ext_data_control_manager_v1");
            display_.reset();
            return false;
        }

        device_ = ext_data_control_manager_v1_get_data_device(
            display_->manager_ext(), display_->seat());
        if (device_ == nullptr) {
            LH_LOG_ERROR("ext_data_control_manager_v1_get_data_device returned null");
            display_.reset();
            return false;
        }
    ...
    void SetClipboardText(const std::string& utf8) override {
        std::ostringstream m;
        m << "[ext] SetClipboardText(len=" << utf8.size() << ")";
        LH_LOG_INFO(m.str());
    ...
    void SetClipboardImage(const std::vector<std::uint8_t>& webp_bytes) override {
        std::ostringstream m;
        m << "[ext] SetClipboardImage(len=" << webp_bytes.size() << ")";
        LH_LOG_INFO(m.str());
    ...
    void AnnounceDelayedText(const std::string& content_hash) override {
        std::ostringstream m;
        m << "[ext] AnnounceDelayedText(hash=" << ShortHash(content_hash) << ")";
        LH_LOG_INFO(m.str());
    ...
    void AnnounceDelayedImage(const std::string& content_hash) override {
        std::ostringstream m;
        m << "[ext] AnnounceDelayedImage(hash=" << ShortHash(content_hash) << ")";
        LH_LOG_INFO(m.str());
    ...
    void AnnounceDelayedFiles(const std::string& content_hash) override {
        std::ostringstream m;
        m << "[ext] AnnounceDelayedFiles(hash=" << ShortHash(content_hash) << ")";
        LH_LOG_INFO(m.str());
    ...
    void ProvideData(const std::string&            content_hash,
                     const std::vector<std::uint8_t>& data) override {
        {
            ...
            if (state_->current_hash != content_hash) {
                std::ostringstream m;
                m << "[ext] ProvideData hash mismatch (pending="
...
private:
    ...
    void TeardownWaylandObjects() {
        ...
        for (auto& kv : offer_mimes_) {
            ext_data_control_offer_v1_destroy(kv.first);
        }
        offer_mimes_.clear();
        if (current_source_ != nullptr) {
            ext_data_control_source_v1_destroy(current_source_);
    ...
    void RecreateSource(ContentKind kind) {
        ...
        current_source_ = ext_data_control_manager_v1_create_data_source(
            display_->manager_ext());
        if (current_source_ == nullptr) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ext_data_control_source_v1_offer(current_source_, mimes[i]);
        }

        ext_data_control_device_v1_set_selection(device_, current_source_);
        display_->Flush();
    }

    // ── ext-data-control source events ──
    ...
    void OnSourceSend(const char* mime, int fd) {
        ...
        lm << "[ext] send event (mime=" << (mime ? mime : "<null>")
    ...
    void OnSourceCancelled() {
        InvokeOnMain([this] {
            if (current_source_ != nullptr) {
                ext_data_control_source_v1_destroy(current_source_);
                current_source_ = nullptr;
                LH_LOG_DEBUG("[ext] source cancelled by compositor; destroyed");
            }
        });
    }

    // ── ext-data-control device events ──

    void OnDeviceDataOffer(struct ext_data_control_offer_v1* offer) {
        if (offer == nullptr) return;
        ext_data_control_offer_v1_add_listener(offer, &kOfferListener, this);
        offer_mimes_[offer] = {};
    }

    void OnOfferMime(struct ext_data_control_offer_v1* offer,
                     const char* mime) {
    ...
    std::unordered_set<std::string> DrainOtherOffers(
            struct ext_data_control_offer_v1* keep) {
        ...
        for (auto it = offer_mimes_.begin(); it != offer_mimes_.end();) {
            if (it->first == keep) {
                ...
                ext_data_control_offer_v1_destroy(it->first);
    ...
    void OnDeviceSelection(struct ext_data_control_offer_v1* offer) {
        ...
        ext_data_control_offer_v1_receive(offer, chosen, pipefd[1]);
        ::close(pipefd[1]);
    ...
    // ── ext-data-control listener tables (must be visible to C) ──

    static void S_SourceSend(void* data,
                             struct ext_data_control_source_v1*,
                             const char* mime, int32_t fd) {
        static_cast<ExtClipboardManager*>(data)->OnSourceSend(mime, fd);
    }
    static void S_SourceCancelled(void* data,
                                  struct ext_data_control_source_v1*) {
        static_cast<ExtClipboardManager*>(data)->OnSourceCancelled();
    }
    static const struct ext_data_control_source_v1_listener kSourceListener;

    static void S_DeviceDataOffer(void* data,
                                  struct ext_data_control_device_v1*,
                                  struct ext_data_control_offer_v1* offer) {
        static_cast<ExtClipboardManager*>(data)->OnDeviceDataOffer(offer);
    }
    static void S_DeviceSelection(void* data,
                                  struct ext_data_control_device_v1*,
                                  struct ext_data_control_offer_v1* offer) {
        static_cast<ExtClipboardManager*>(data)->OnDeviceSelection(offer);
    }
    static void S_DeviceFinished(void* data,
                                 struct ext_data_control_device_v1*) {
        (void)data;
        LH_LOG_WARN("[ext] data_control_device finished by compositor");
    }
    ...
    static const struct ext_data_control_device_v1_listener kDeviceListener;

    static void S_OfferMime(void* data,
                            struct ext_data_control_offer_v1* offer,
                            const char* mime) {
        static_cast<ExtClipboardManager*>(data)->OnOfferMime(offer, mime);
    }
    static const struct ext_data_control_offer_v1_listener kOfferListener;

    // ── State ──

    std::shared_ptr<ExtState>       state_;
    std::unique_ptr<WaylandDisplay>     display_;
    struct ext_data_control_device_v1* device_{nullptr};
    struct ext_data_control_source_v1* current_source_{nullptr};

    // Per-offer MIME accumulator; lifetime tied to the manager (only
    // touched from the main thread, no lock).
    std::unordered_map<struct ext_data_control_offer_v1*,
                       std::unordered_set<std::string>> offer_mimes_;
};

const struct ext_data_control_source_v1_listener
    ExtClipboardManager::kSourceListener = {
        /*send=*/      &ExtClipboardManager::S_SourceSend,
        /*cancelled=*/ &ExtClipboardManager::S_SourceCancelled,
};

const struct ext_data_control_device_v1_listener
    ExtClipboardManager::kDeviceListener = {
        /*data_offer=*/        &ExtClipboardManager::S_DeviceDataOffer,
        /*selection=*/         &ExtClipboardManager::S_DeviceSelection,
        /*finished=*/          &ExtClipboardManager::S_DeviceFinished,
        /*primary_selection=*/ &ExtClipboardManager::S_DevicePrimarySelection,
};

const struct ext_data_control_offer_v1_listener
    ExtClipboardManager::kOfferListener = {
        /*offer=*/ &ExtClipboardManager::S_OfferMime,
};

std::unique_ptr<ClipboardManager> MakeExtManager() {
    auto m = std::make_unique<ExtClipboardManager>();
    if (!m->Init()) {
        LH_LOG_ERROR("ExtClipboardManager init failed");
        return nullptr;
    }
    return m;
}
